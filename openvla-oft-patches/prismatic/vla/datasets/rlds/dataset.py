...
def make_dataset_from_rlds(
    ...
    *,
    ...
    # Try to load TFDS builder; fall back to direct TFRecord reading for custom datasets
    import tensorflow_datasets as tfds
    try:
        builder = tfds.builder(name, data_dir=data_dir)
        use_direct_tfrecord = False
    except Exception:
        # Custom dataset not registered with TFDS - use direct TFRecord reading
        overwatch.info(f"TFDS builder not found for '{name}', falling back to direct TFRecord reading")
        use_direct_tfrecord = True
        builder = None  # Will use dlimp directly instead
    ...
    if isinstance(dataset_statistics, str):
        ...
        if use_direct_tfrecord:
            # Import custom LIBERO reader
            from prismatic.vla.datasets.rlds.libero_reader import load_libero_dataset
            full_dataset = load_libero_dataset(
                data_dir, name, shuffle=False, num_parallel_reads=num_parallel_reads
    ...
    if use_direct_tfrecord:
        # Read TFRecords directly using our custom reader (returns a DLataset)
        from prismatic.vla.datasets.rlds.libero_reader import load_libero_dataset
        dataset = load_libero_dataset(
            data_dir, name, shuffle=shuffle, num_parallel_reads=num_parallel_reads
        )
    else:
